...
router.get("/", authRequired, async (req, res) => {
  try {
    const { from, to, category, search, page, limit: limitStr, excludeSavings, includeAuto } = req.query;
    ...
    // Exclude auto-generated bookkeeping expenses (created by
    // utils/billPaymentService.js when a bill is paid in a different
    // pay period than its due date — they represent bill outflows,
    // not user-logged spending). Opt in with ?includeAuto=true for
    // debugging/admin tools. This is what stops paid-early bill
    // entries like "Burbank Hotel (paid early)" from showing up in
    // the Calendar day sheet and ExpenseHistory lists.
    if (includeAuto !== "true" && includeAuto !== "1") {
      query.autoGenerated = { $ne: true };
    }
